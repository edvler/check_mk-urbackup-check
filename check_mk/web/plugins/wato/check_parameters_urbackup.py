...
register_check_parameters(
    ...
    Dictionary(
        elements = [
            ...
            ('backup_age',
             Tuple(
                 title = "Age of Backup before changing to warn or error (this parameters are only used, if - Backup age - modi is choosen!)",
                 elements = [
                     Age(title=_("Warning at or above backupage"),
                         default_value = 93600,
                         display = [ "minutes" ],
                         help=_("If the backup is older as the specified time, the check changes to warning. (24h=1440m; 26h=1560m)")
                     ),
